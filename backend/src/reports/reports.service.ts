...
export class ReportsService {
  ...
  async getPnlDistribution() {
    ...
    const averageWin =
      winningTrades > 0 ? Number((grossProfit / winningTrades).toFixed(2)) : 0;
    const averageLoss =
      losingTrades > 0 ? Number((grossLoss / losingTrades).toFixed(2)) : 0;
    const profitFactor =
      grossLoss > 0
  ...
  async getProfitReport(userId: string) {
    ...
    const totalDistributed = paid.reduce(
      (s, d) => s + Number(d.netProfit ?? 0),
      0,
    );
  ...
  async getWalletStatement(userId: string) {
    ...
    const profitCredits = distributions.reduce(
      (s, d) => s + Number(d.netProfit ?? 0),
      0,
    );
    const closingBalance = wallet ? Number(wallet.realizedBalance) : 0;
    const openingBalance =
      closingBalance - totalDeposits + totalWithdrawals - profitCredits;
  ...
  async getTaxSummary(userId: string) {
    ...
    const distributionIncome = distributions.reduce(
      (s, d) => s + Number(d.netProfit ?? 0),
      0,
    );
    const totalRealizedGains = Number(
      (tradingPnL + distributionIncome).toFixed(2),
    );
    const taxRate = 0.15;
  ...
  async saveReportRecord(
    ...
    if (!user) {
      console.warn(
        `User ${userId} not found, skipping report record generation.`,
      );
      return null;
    }
  ...
  async generateCsvBuffer(
    ...
    if (type === 'trading') {
      ...
      const rows = data.records
        .map(
          (r) =>
            `${r.pair},${r.side},${r.entryPrice},${r.exitPrice},${r.result},${r.profitLoss},${r.tradeDate}`,
        )
        .join('\n');
      return {
        buffer: Buffer.from(header + rows, 'utf-8'),
        fileName: `Forex_Auto_Panel_Trading_Report_${dateStr}.csv`,
      };
    }
    ...
    if (type === 'profit') {
      ...
      const rows = data.records
        .map(
          (r) =>
            `${r.reference},${r.amount},${r.type},${r.status},${r.distributionDate},"${r.note}"`,
        )
        .join('\n');
      return {
        buffer: Buffer.from(header + rows, 'utf-8'),
        fileName: `Forex_Auto_Panel_Profit_Report_${dateStr}.csv`,
      };
    }
    ...
    if (type === 'wallet') {
      ...
      const depositRows = data.deposits.map(
        (d) => `${d.date},Deposit (${d.plan}),${d.amount},`,
      );
      const withdrawRows = data.withdrawals.map(
        (w) => `${w.date},Withdrawal,,${w.amount}`,
      );
      const profitRows = data.profitEntries.map(
        (p) => `${p.date},Profit Credit - ${p.reference},${p.amount},`,
      );
      return {
        buffer: Buffer.from(
          header + [...depositRows, ...withdrawRows, ...profitRows].join('\n'),
          'utf-8',
        ),
        fileName: `Forex_Auto_Panel_Wallet_Statement_${dateStr}.csv`,
      };
    }
    ...
    if (type === 'tax') {
      ...
      return {
        ...
        fileName: `Forex_Auto_Panel_Tax_Summary_${dateStr}.csv`,
      };
    }
  ...
  async generatePdfBuffer(
    ...
    let tradingData: Awaited<ReturnType<typeof this.getTradingReport>> | null =
      null;
    let profitData: Awaited<ReturnType<typeof this.getProfitReport>> | null =
      null;
    let walletData: Awaited<ReturnType<typeof this.getWalletStatement>> | null =
      null;
    ...
    const buffer = await new Promise<Buffer>((resolve, reject) => {
      try {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fillColor('#4ade80')
          .text('FOREX AUTO PANEL', 50, 18, { lineBreak: false });
        doc
          .fontSize(8)
          .font('Helvetica')
          .fillColor('#86efac')
        ...
        if (type === 'trading' && tradingData) {
          ...
          if (tradingData.records.length > 0) {
            drawSection(
              doc,
              `Trade Records (${Math.min(40, tradingData.records.length)} shown)`,
            );
          ...
          doc
            ...
            .fillColor('#9ca3af')
            .text(
              'DISCLAIMER: Estimated at 15% STCG. Consult a certified chartered accountant for official filings.',
            );
        }
        ...
        doc
          ...
          .fillColor('#aaaaaa')
          .text(
            'Generated by Forex Auto Panel Financial Reports Platform. System-generated document.',
            50,
            doc.page.height - 45,
    ...
    const fileNames: Record<string, string> = {
      trading: `Forex_Auto_Panel_Trading_Report_${dateStr}.pdf`,
      profit: `Forex_Auto_Panel_Profit_Report_${dateStr}.pdf`,
      wallet: `Forex_Auto_Panel_Wallet_Statement_${dateStr}.pdf`,
      tax: `Forex_Auto_Panel_Tax_Summary_${dateStr}.pdf`,
    };

    return {
      buffer,
      fileName: fileNames[type] ?? `Forex_Auto_Panel_Report_${dateStr}.pdf`,
    };
  }
}
